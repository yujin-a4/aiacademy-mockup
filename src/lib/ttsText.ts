...
const TERM_KO: Array<[RegExp, string]> = [
  ...
  * ⚠️ **낱말 속 ing 는 절대 건드리면 안 된다** — 낱말 경계를 빼면 "painting" 이 "pa아이엔쥐" 가 된다. */
  ...
  [/\s*\+\s*/g, ' 플러스 '],
  /* ── `p.p.` 의 **마침표를 뗀다** (09-04 실측) ──
     v3 는 마침표를 문장 끝으로 본다. "has/have been p.p.는 be p.p.랑 …" 한 줄에 마침표가
     넷이라 모델은 **다섯 문장**을 읽는 셈이 되어 뚝뚝 끊겼다. 같은 문장을 세 번씩 뽑아 재보니
     6.04초 → 4.18초(31% 단축)였고, 3회 편차(±0.2초)보다 훨씬 크다.
     ⚠️ 한글로 바꾸는 것(`피피`)은 오히려 느렸다(6.06초). 대문자 두 글자가 가장 매끄럽다.
