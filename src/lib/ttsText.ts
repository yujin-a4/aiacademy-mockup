...
const TERM_KO: Array<[RegExp, string]> = [
  /* ── 'be동사' 는 **통째로 한글로** 읽는다 (메모 118행) ──
     발음 사전이 `be` 를 IPA 로 바꾸면 "/biː/동사" 가 되는데, v3 가 이걸 [비도동사] 로 읽는다는
     보고가 두 번 왔다(윤다은). IPA 표의 본업은 "한글이 바로 붙는 자리" 인데 **바로 그 자리가**
     탈이 났다 — 슬래시와 한글이 맞붙으면 모델이 경계를 못 읽는다.
     여기서 통째로 바꿔 두면 `be` 가 남지 않으므로 applyPronunciation 이 손댈 것이 없다.
     ⚠️ TERM_KO 는 **모델을 가리지 않는다** — v2 를 쓰는 강사에게도 같이 걸리는데, 그쪽도
        [비동사] 가 맞는 발음이라 문제되지 않는다. */
  [/\bbe\s*동사/gi, '비동사'],
