...
import { TipCard, ConceptBoard, TipButtons, TipSheet, type SeenTip, type TipSheetKind } from '@/components/type-lesson/TipNotes'
...
/** 판정 결과 — `O` 맞음 · `X` 틀림 · `P` 덜 말함 · `Q` 강사에게 묻는 말 · `N` 딴소리 · `?` 판정 못 함 */
type Verdict = 'O' | 'X' | 'P' | 'Q' | 'N' | '?'

/** ── 뜻으로 판정한다 — 화면이 낱말 겹침으로 못 가린 것만 여기로 온다 ──
 *
 *  수업 턴(judgeSubjective)과 마지막 정리 화면이 **같은 판정기를 쓴다.** 정리 화면은 시트가
 *  한 칸에 답을 여럿 적어 두는 자리가 있어서("주어와 동사의 의미 관계, 의미") 글자 겹침만으로는
 *  못 가린다 — 어느 쪽으로 말해도 맞다고 해야 한다.
 *
 *  ⚠️ 실패·타임아웃은 **O 로 떨어뜨리지 않는다.** 맞은 답을 틀렸다 하는 것이 더 나쁘다는
 *     이유로 그랬다가, 자판을 누른 자국("ㅇㅁㄴㄹㄹ")에도 "좋습니다" 가 나갔다(실측).
 *     못 했으면 못 했다고 하고, 부르는 쪽이 중립으로 넘긴다.
 *  ⚠️ 9초까지 기다린다. 로컬 실측 2.6초인데 태블릿 첫 호출이 3.5초를 자주 넘겨, 끊기면 '?' 가
 *     되어 **맞은 답이 조용히 넘어갔다.** 몇 초 기다리는 쪽이 낫다. */
async function askJudge(question: string, expected: string, said: string): Promise<Verdict> {
  ...
  try {
    const res = await fetch('/api/gemini', {
      ...
      body: JSON.stringify({
        judge: true,
        /* 학생 답은 **음성인식 전사**일 수 있다 — 소리로 깨진 영어 용어를 되돌린 추정본도
           같이 보낸다(둘 다 주고 판정기가 고르게 한다). 원문만 보내면 "피피피" 가 오답이 된다. */
        message: [
          `[강사 질문] ${question}`,
          `[기대 답] ${expected}`,
          `[학생 답] ${said}`,
          fixSttJargon(said) !== said ? `[소리 보정 추정] ${fixSttJargon(said)}` : '',
        ].filter(Boolean).join('\n'),
      }),
    })
    ...
    /* 브라우저에서도 보이게 남긴다 — 서버 터미널 로그와 짝이다(api/gemini 의 같은 줄).
       "맞게 말했는데 틀렸다" 를 쫓을 때 **판정기에 들어간 글자**부터 봐야 한다. */
    console.log('[judge] 학생 답:', JSON.stringify(said), '· 보정:', JSON.stringify(fixSttJargon(said)),
      '· 기대:', JSON.stringify(expected), '→', v)
    for (const k of ['Q', 'N', 'X', 'P', 'O'] as const) if (v.startsWith(k)) return k
...
/* ── 음성인식이 **영어 문법 용어를 소리대로 적어 올 때** (09-18) ──
   한국어 인식기(ko-KR)에 "be p.p." 를 말하면 영어 낱말이 아니라 소리로 떨어진다:
   "피피피" · "PPPP" · "비피피" · "삐삐". 학생은 정확히 말했는데 화면은 못 알아들은 것이 되고,
   채점기도 기대 답("be p.p.")과 한 글자도 안 겹치니 오답으로 몰았다(실측 09-18).
   맞춰 보기 **전에** 소리를 용어로 되돌린다. 기록·말풍선에는 학생이 한 말 그대로 남긴다.
   ⚠️ 여기 넣는 것은 **수업에서 실제로 시키는 말**뿐이다. 넓게 잡을수록 엉뚱한 답이 정답으로
      둔갑한다 — 뜻을 고쳐 주는 자리가 아니라 소리를 되돌리는 자리다. */
function fixSttJargon(t: string): string {
  return t
    /* 비/be + 피피(피…) → be p.p. (앞의 '비' 는 be 를 소리로 적은 것이다) */
    .replace(/(?:비|be)\s*[피삐]\s*[피삐](?:\s*[피삐])*/gi, 'be p.p.')
    .replace(/(?:비|be)\s*p\.?\s*p\.?/gi, 'be p.p.')
    /* 홀로 온 피피·PPP… → p.p. */
    .replace(/[피삐]\s*[피삐](?:\s*[피삐])*/g, 'p.p.')
    .replace(/\bp[\s.]*p[\s.p]*/gi, 'p.p.')
    /* being — 인식기가 영어로 적어 오기도 한다("Bing"), 한글로 적어 오기도 한다("비잉"·"빙").
       "be being p.p." 를 말하면 앞의 be 가 붙어 **"Bing 피피"** 한 덩어리로 떨어진다(실측 09-18). */
    .replace(/(?:비|be)\s*(?:빙|잉|bing)/gi, 'being')
    .replace(/bing/gi, 'being')
    /* ⚠️ 뒤돌아보기(lookbehind)를 쓰지 않는다 — 옛 사파리는 그 정규식을 **읽다가 터진다**(파일 전체가
       안 뜬다). 앞 글자는 붙잡아 두었다가 그대로 돌려준다. */
    .replace(/(^|[^가-힣])빙(?=$|[^가-힣])/g, '$1being')
    .replace(/해브\s*빈/g, 'have been')
    .replace(/이즈\s*being/gi, 'is being')
    .replace(/\s+/g, ' ')
    .trim()
}

function subjectiveOk(said: string, expected?: string, accepts?: string[]): boolean {
  said = fixSttJargon(said)
...
/** 한국어 STT 가 적어 오는 보기 이름 — 'B' 는 '비'·'B'·'2번' 으로 온다 */
...
/** 말한 것이 **몇 번 보기인가** — 라벨·번호·보기 글자로 찾는다. 없거나 애매하면 -1.
 *  애매한 것을 임의로 정하지 않는 것은 matchTypedChoice 와 같다(부르는 쪽이 질문으로 넘긴다). */
function matchSpokenLabel(text: string, opts: { label: string | null; text: string }[]): number {
  const norm = (s: string) => s.toLowerCase().replace(/[\s.,!?'"·]/g, '')
  const t = norm(text)
  if (!t) return -1
  /* 라벨이 없는 보기('없어요' — 더 들을 것이 없다) */
  const none = opts.findIndex((o) => !o.label)
  if (none >= 0 && /^(없어요|없어|없음|없습니다|괜찮아요|괜찮아|아니요|아니오)$/.test(t)) return none
  /* "비요"·"2번이요"·"디인 것 같아요" — 말끝을 떼고 본다 */
  const bare = stripTail(t)
  const byLabel = opts.findIndex((o) => o.label && (LABEL_ALIAS[o.label.toUpperCase()] ?? []).some((a) => a === t || a === bare))
  if (byLabel >= 0) return byLabel
  const hits = opts.map((o, i) => ({ i, c: norm(o.text) }))
...
/** 말끝을 뗀다 — 보기를 말할 때 학생은 "비" 라고만 하지 않는다("비요", "비 같아요", "2번이요").
 *  맞춰 볼 때만 쓴다(기록에는 학생이 한 말 그대로 남긴다). */
function stripTail(t: string): string {
  let s = t
  for (let i = 0; i < 3; i++) {
    const cut = s.replace(/(인것같아요|인것같습니다|같아요|같습니다|이에요|예요|이요|요|입니다|번|인듯|일듯)$/, '')
    if (cut === s) break
    s = cut
  }
  return s
...
/** **물어본 것인가** — 보기가 떠 있는 자리에서 말한 것은 원칙적으로 답이다.
 *  다만 진짜 질문("이거 무슨 뜻이에요?")까지 답으로 채점하면 엉뚱한 오답이 된다.
 *  그래서 의문 표시가 뚜렷할 때만 질문으로 넘긴다 — 애매하면 **답으로 본다.** */
function looksLikeQuestion(text: string): boolean {
  const t = text.trim()
  if (/[?？]\s*$/.test(t)) return true
  return /(무슨|무엇|뭐|뭔|왜|어떻게|어떤|어디|언제|누가|뜻이|의미|차이|알려주|설명해|맞나요|맞아요\?|인가요|일까요|인지)/.test(t)
}
...
function directiveOf(turn: Turn, gate: Gate = 4): string {
  ...
  return [
    `[단계] ${turn.stage}`,
    `[이번 턴에 전달할 내용] ${stripAudioTags(turn.tutor)}`,
...
const KIND_HEADING: Record<Interaction['kind'], string> = {
  ...
  subjective: '생각 말하기', mark: '단서 찾기', match: '근거 연결', askOption: '더 볼 보기 고르기',
}
...
export default function TypeLessonPlayer({ lesson: lessonProp, instructor = RAIL_OWNER, lectureCode, lectureTitle, draftId, preparing, initialStage, scripted, scriptedReview, scriptedPracticeOutro, scriptedIntro, scriptedSummary, scriptedConceptTip }: {
  lesson: TypeLesson
  instructor?: string
  ...
  /** 개념 학습 구간에 크게 띄우는 **토익 TIP 판** (시트 '개념 학습' 의 TIP 칸).
   *  `( ① ______ )` 가 빈칸인 채로 먼저 뜨고, 턴의 `tipAt` 이 가리키는 자리가 하나씩 열린다.
   *  구간 끝 턴에는 다 채운 TIP 이 `turn.tip` 으로 붙어 있어, 그대로 사이드 노트로 접힌다. */
  scriptedConceptTip?: LessonTip
  ...
  /* ── 후속 질문 · 오답 해설 골라 듣기 (09-03, 이도윤 개념학습본) ──
     예전 대본은 오답 보기 셋을 순서대로 다 짚었다. 새 대본은 **학생이 고른 오답 하나**를 먼저
     짚고, 그다음 "더 듣고 싶은 보기 있나요?" 를 물어 고른 것만 짚는다. 그래서 대본에는 같은
     S6 해설이 두 벌 들어 있고(생성기 orderTurns), 화면이 조건에 안 맞는 턴을 건너뛴다.
     후속 질문이 없는 대본(윤다은·이도윤 RC)은 gate 가 하나도 없어 이 장치가 통째로 잠잔다. */
  /** heard — 이미 들은 오답 해설 `${qIdx}:${label}` (후속 질문 목록에서 뺀다)
   *  want  — 후속 질문에서 지금 고른 보기. 그 보기의 onDemand 해설만 열린다
   *  done  — '없어요' 를 골랐거나 더 들을 것이 없는 문항. 다시 묻지 않는다
   *
  ...
  const resetAsk = () => { askRef.current = { heard: new Set(), want: {}, done: new Set() }; bumpAsk() }

  /** 이 문항에서 학생이 **틀리게 고른 보기**. 맞혔으면 null — 그러면 ifPicked 해설은 하나도 안 열린다
   *  (맞힌 학생은 후속 질문으로 고른 것만 듣는다. 이게 새 대본의 의도다). */
  /** 이 문항에서 학생이 **처음 고른 보기**. 다시 골라도 덮지 않는다.
   *
   *  ⚠️ 이게 없으면 **다시 풀리는 대본이 스스로를 지운다**(09-04 정오답분기, 윤다은).
   *  그 대본은 틀린 학생을 오답 경로로 데려가 답을 다시 고르게 하고, 그 뒤에
   *  "(재선택) 맞아요! …" 를 말한다. 그런데 길을 고르는 기준이 `answers`(지금 고른 것)면,
   *  다시 골라 맞히는 순간 "이 학생은 맞혔다" 가 되어 **남은 오답 경로 턴이 통째로 닫힌다** —
  ...
  /** 이 턴을 지금 트는가 — gate 가 없으면 늘 튼다(대본 대부분이 그렇다) */
  const shouldPlay = (t?: Turn): boolean => {
    if (!t) return false
    if (t.interaction.kind === 'askOption') {
      /* 더 들을 보기가 없으면 묻지 않는다 — "있나요?" 해놓고 '없어요' 하나만 내밀면 묻는 시늉이다 */
      return !askRef.current.done.has(t.focusQ ?? -1) && askableOf(t).some((c) => c.label)
    }
    ...
    /* ── 채점 뒤 갈래 (RC) ──
       ...
       아직 안 푼 자리(answers 가 빈 자리)는 **맞힌 쪽**으로 본다 — 짧은 길이 안전하다. */
  ...
  const seekPlayable = (i: number) => { let j = i; while (j < turns.length && !shouldPlay(turns[j])) j += 1; return j }

  /** 앞뒤 이웃 턴 — **실제로 트는 것만** 센다.
   *
   *  gate 가 생긴 뒤로 바로 옆 턴이 건너뛸 턴일 수 있다. 그런데 "다음 줄이 맞장구로 시작하나"
   *  (scriptWillAck), "다음 줄이 답을 말해 주나"(scriptWillTell), "앞 턴이 학생에게 물었나"
   *  (정답·오답 갈래 고르기) 는 전부 **들려주는 줄**을 두고 하는 판단이다. 순번으로 옆을 보면
   *  화면이 건너뛴 줄을 보고 정하게 된다 — 윤다은 정오답분기에서 채점 턴이 두 벌이라
   *  틀린 학생 자리에서 맞힌 학생 몫의 줄을 읽고 판단했다(09-04).
   *  갈래가 없는 대본에서는 늘 바로 옆이 나오므로 예전과 같다. */
  const playedNeighbor = (from: number, dir: 1 | -1): Turn | undefined => {
    for (let i = from + dir; i >= 0 && i < turns.length; i += dir) {
      if (shouldPlay(turns[i])) return turns[i]
    }
    return undefined
  }
  ...
  /** 입력 모드에서 **눌러서 말하는 중인가.** 모드를 바꾸면 꺼진다 — 음성 모드는 늘 듣고 있어서
   *  이 값이 켜진 채로 남으면 나중에 입력 모드로 돌아왔을 때 마이크가 이미 열려 있다. */
  const [pushTalk, setPushTalk] = useState(false)
  useEffect(() => { setPushTalk(false) }, [chatMode])
  ...
  const conversation = useConversation({
    /* ── 입력 모드라고 마이크를 **아주** 닫지는 않는다 (09-17 개편) ──
       예전에는 `chatMode === 'text'` 면 무조건 음소거였다. 그래서 입력 모드에서 말로 답한
       참가자에게는 "말해도 아무 일이 없는 화면" 이 됐고, 그걸 4강 내내 반복했다(FGI 행동관찰).
       이제 입력 모드에서도 **마이크 버튼을 누르는 동안은** 듣는다. 계속 열어 두지는 않는다 —
       그러면 혼잣말·주변 소리가 답으로 들어간다. */
    micMuted: chatMode === 'text' && !pushTalk,
  ...
  /* ── 개념 학습의 **TIP 판** (09-18 시트 개정) ──
     문항이 아직 없는 구간이라 화면에는 사진 대신 이 판이 크게 뜬다. 판은 빈칸이 뚫린 채로
     시작하고, 강사가 그 자리에 닿는 턴(`tipAt`)에서 한 칸씩 열린다.
     **상태로 들지 않고 `turnIdx` 로 매번 다시 센다** — 되감으면 같이 닫혀야 맞다(TIP 노트와 같은 규칙).
     구간 끝 턴에는 다 채운 TIP 이 `turn.tip` 으로 붙어 있어 거기서부터는 평소 카드가 그린다. */
  const boardOpen: Record<number, string> | null = useMemo(() => {
    if (!scriptedConceptTip || !turn.board) return null
    /* ── 칸은 그 단계가 **끝나야** 열린다 ──
       예전에는 턴이 시작하자마자 열었다. 그러면 강사가 아직 설명을 꺼내지도 않았는데 답이 먼저
       화면에 뜬다 — 묻는 턴에서는 **정답을 미리 보여주는 것**이 되어버린다("…어떤 표현을 쓸까요?"
       옆에 이미 be p.p. 가 떠 있었다). 그래서 둘로 갈라 본다:
  ...
  const boardFull: Record<number, string> | null = useMemo(() => {
    if (!scriptedConceptTip) return null
    const open: Record<number, string> = {}
    for (const t of turns) if (t.board) for (const r of t.tipAt ?? []) if (r.text) open[r.n] = r.text
    return open
  }, [scriptedConceptTip, turns])

  /* ── 개념 학습이 끝나면 **판째로 접혀 들어간다** ──
     예전에는 판이 툭 사라지고 그 자리에 없던 쪽지가 튀어나와 날아갔다 — 학생이 방금 읽던 것과
     날아가는 것이 다른 물건이라 "저기에 저장됐다" 로 안 읽혔다. 이제 판 자체가 줄어들고,
     **그 아래에서 유형 학습 1번 문항이 드러난다.** 쪽지 쪽 애니메이션은 그동안 비켜선다. */
  ...
  /** 이 문항을 **아직 한 번 더 풀린다** — 뒤에 남은 '정답 고르기' 턴이 있는가 (09-04).
   *
   *  윤다은 정오답분기 대본은 틀린 학생을 오답 경로로 데려가 S5 에서 답을 다시 고르게 한다.
   *  그런데 정오답 색은 그 **앞**인 채점 턴에서 켜진다(TELLS_ANSWER 에 `^채점` 이 있다).
   *  켜지면 정답이 화면에 그대로 보여서 **다시 고를 것이 없어진다** — 다시 풀리는 의미가 사라진다.
   *  그래서 뒤에 다시 고를 자리가 남아 있으면 그때까지 감춘다.
   *
   *  ⚠️ **틀 수 있는 턴만 센다.** 맞힌 학생에게는 그 재선택 턴이 gate 로 꺼져 있으므로
   *     저절로 예전처럼 채점에서 열린다 — 맞힌 학생까지 답을 못 보게 되면 안 된다.
   *  ⚠️ 다시 풀리는 자리가 없는 대본(윤다은 간결·이도윤)에서는 늘 false 라 예전 그대로다. */
  const willAskAgain = (qIdx?: number): boolean => {
    if (qIdx === undefined || phase === 'review') return false
    /* ⚠️ **지금 턴부터** 센다(turnIdx + 1 이 아니다). 다시 고르라는 그 턴에 도착하는 순간
       색이 켜지면 학생은 답을 본 채로 고르게 된다 — 재선택 턴 자신이 가장 감춰야 할 자리다. */
    return turns.slice(turnIdx).some((t) => t.interaction.kind === 'pickAnswer'
      && t.interaction.qIdx === qIdx && shouldPlay(t))
  }
  ...
  useEffect(() => {
    ...
    /* 아직 한 번 더 풀릴 문항이면 열지 않는다 — 열면 다시 고를 것이 없다 (willAskAgain) */
    if (willAskAgain(q)) return
  ...
  const { revealedScript, revealedOptions, activePassageId } = useMemo(() => {
    ...
    for (let i = 0; i <= turnIdx && i < turns.length; i++) {
      /* ── **건너뛴 턴은 아무것도 열지 않는다** (09-04) ──
         이 반복문은 지나온 턴을 순번으로 훑는데, gate 가 생긴 뒤로 그 사이에 **틀지 않은 턴**이
         섞인다. 윤다은 정오답분기에서 틀린 학생은 3번(채점·맞힘 갈래)을 건너뛰고 4번으로 가는데,
         3번의 reveal 이 정답 보기 B 를 열고 있어서 **틀리자마자 정답 스크립트가 열렸다**(실측 09-04).
         학생이 듣지도 않은 줄이 화면을 여는 셈이라, 틀 수 없는 턴은 여기서 빼야 맞다. */
      if (!shouldPlay(turns[i])) continue
  ...
  useEffect(() => {
    ...
    if (phase !== 'lesson' && phase !== 'review') return
    /* 오답 해설을 연 자리면 '들었다' 고 표시한다 — 후속 질문이 같은 보기를 다시 내밀지 않게.
       ref 에 곧바로 적는다(아래 자동 전진이 이 값을 같은 흐름 안에서 다시 읽는다). */
    const gated = turns[turnIdx]
    if (gated?.optionRef && (gated.gate === 'ifPicked' || gated.gate === 'onDemand') && gated.focusQ !== undefined) {
      const key = `${gated.focusQ}:${gated.optionRef}`
      if (!askRef.current.heard.has(key)) { askRef.current.heard.add(key); bumpAsk() }
    }
    /* ── 보기 해설이 **왜 나왔는지/왜 안 나왔는지** 한 줄로 남긴다 (09-18) ──
       "정답이면 정답 보기만, 오답이면 그 보기만, 그다음 나머지를 묻는다" 가 어긋나 보일 때
       눈으로 쫓을 수 있어야 한다. 갈림은 셋뿐이다: 이 턴의 gate · 학생이 틀리게 고른 보기
       (wrongPickOf) · 이미 들은 보기(heard). 셋을 같이 찍는다. */
    if (gated?.gate || gated?.interaction.kind === 'askOption') {
      const q = gated.focusQ ?? -1
      console.log('[rail]', `턴 ${turnIdx}`, gated.interaction.kind,
        `gate=${gated.gate ?? '-'}`, `보기=${gated.optionRef ?? '-'}`, `문항=${q}`,
        `학생오답=${wrongPickOf(gated.focusQ) ?? '없음(정답)'}`,
        `이미들음=[${Array.from(askRef.current.heard).join(',')}]`,
        gated.interaction.kind === 'askOption'
          ? `남은보기=[${askableOf(gated).map((c) => c.label ?? '없어요').join(',')}]` : '')
    }
    ...
    ;(async () => {
      ...
      if (scripted) {
        ...
        const askedJustBefore = ASKING_KINDS.has(playedNeighbor(turnIdx, -1)?.interaction.kind ?? '')
      ...
      if (alive && !askingRef.current && (scripted || agentOnRef.current) && !needsAnswer(turn) && nextIdxFrom(turnIdx) < turns.length && !atItemEnd) {
        ...
        if (alive && !askingRef.current && turnIdxRef.current === turnIdx) advanceByApp(nextIdxFrom(turnIdx))
      }
    })().catch(() => {
  ...
  const goNext = () => {
    /* 문제 경계에서는 아무 일도 하지 않는다 — 넘기는 것은 [다음 문제] 버튼 하나뿐이다.
       ...
       화면만 다음 문제로 넘어가 버린다. */
    if (atItemEnd) return
    /* 다음 턴은 **바로 옆이 아닐 수 있다** — 조건이 안 맞는 오답 해설을 건너뛰고,
       해설 하나를 다 들었으면 후속 질문으로 되돌아간다(nextIdxFrom) */
    const nx = nextIdxFrom(turnIdx)
    if (nx < turns.length) { setTurnIdx(nx); return }
    stopVoice()
    /* ── 수업(스캐폴딩) 끝 → **바로 실전** (사용자 지시 09-18) ──
       예전에는 사이에 '혼자 들어보는 구간'(freePlay)을 하나 뒀다. 음원을 스스로 눌러 볼 자리를
       주려던 것인데, 실제로는 [유형 학습 마치기] → 안내 문단 → [실전 문제 풀기] 로 **버튼을 두 번**
       누르는 계단이 됐다. 끝났다는 말은 토스트가 이미 하고 있었다.
       리뷰(틀린 문제 다시 풀기)는 그대로 정리로 간다.
       ⚠️ freePlay 자체는 코드에 남아 있다(수업 뒤 자유 질문·번호 이동이 그 상태를 쓴다) —
          다만 **일반 흐름에서는 지나지 않는다.** 되살리려면 여기만 되돌리면 된다. */
  ...
  const startReveal = (raw: string, hasAudio: boolean) => {
    /* ── 흘릴 글자에서 **오디오 태그를 뗀다** ──
       말풍선은 소리를 따라 글자를 하나씩 드러내는데, 그 원본이 태그 붙은 문자열이면
       `[whispers]` 가 잠깐 보였다가 다 흐른 뒤에야 사라진다(실측 09-03).
       흘리기 전에 떼면 애초에 한 번도 안 보인다. 소리로 나가는 문자열은 그대로다 —
       그쪽은 speakTTS 가 따로 받는다. 태그는 읽히지 않으니 글자 수 비율도 이쪽이 더 맞다. */
    const text = stripAudioTags(raw)
    if (revealRef.current) cancelAnimationFrame(revealRef.current)
    const t0 = Date.now()
    let sawAudio = false
  ...
  const scriptWillAck = () => {
    ...
    const nx = playedNeighbor(turnIdxRef.current, 1)
  ...
  /** 이 문항을 틀린 학생이 갈 **길이 대본에 따로 깔려 있는가** (09-04 정오답분기, 윤다은).
   *
   *  위 `scriptWillAnswerWrong` 은 "바로 다음 **한 줄**이 오답까지 받아 주는가" 를 본다.
   *  새 대본은 그 꼴이 아니다 — 오답이면 채점부터 S5 까지 **여러 턴이 통째로 갈린다**
   *  (gate: 'ifWrong'). 그 앞에서 앱이 "A는 아니에요." 를 얹으면 곧바로 대본이
   *  "정답이 아니에요. 핵심 동작부터 다시 잡고…" 를 말해 같은 자리가 두 번이 된다.
   *  길이 깔려 있으면 앱은 비켜서고 대본에 맡긴다.
   *
   *  같은 문항의 턴만 본다 — 뒤 문항의 갈래를 보고 비켜서면 이 문항에서 아무 말도 안 하게 된다.
   *  갈래가 없는 대본(윤다은 간결·이도윤 LC)에서는 늘 false 라 예전 그대로 돈다. */
  const scriptBranchesWrong = (qIdx: number): boolean => turnsRef.current
    .slice(turnIdxRef.current + 1)
    .filter((t) => t.focusQ === qIdx)
    .some((t) => t.gate === 'ifWrong')
  ...
  const scriptWillTell = (): boolean => {
    ...
    return lineCovers(stripAck(playedNeighbor(turnIdxRef.current, 1)?.tutor ?? ''), keys)
  }
  ...
  const handleScriptedAnswer = async (ok: boolean, picked?: string, mode?: 'giveUp' | 'unsure') => {
    ...
    if (ok) {
      ...
      /* ── 맞장구는 **강사가 아니라 대본을 보고** 정한다 (09-03) ──
         예전에는 이도윤을 INST_SCRIPT_ONLY 로 통째로 비켜서게 했다. "그 대본은 줄마다 정오답
         반응을 갖고 있다" 가 그때의 사실이었기 때문이다. **개념학습본이 그 줄들을 지웠다** —
         이제 학생이 2지선다를 눌러도 대본도 앱도 입을 다물어 아무 반응이 없다(실측).
         다음 줄이 받아 주면 비켜서고, 아니면 앱이 말한다. 문구는 강사별 목록에서 고른다.
         옛 대본(이도윤 RC·윤다은)은 다음 줄이 "맞아요…" 라 저절로 예전처럼 비켜선다. */
      if (!scriptWillAck()) await say(ackLine(ackNoRef.current++, instructor))
      goNext()
      return
    }
  ...
  const handleScriptedPick = async (qIdx: number, ok: boolean, why?: string, label?: string) => {
    ...
    /* 해설 경로가 볼 '첫 답' 은 **여기서** 적힌다 — 눌러 본 보기가 아니라 채점된 답이다(위 onSelect) */
    if (label && firstPickRef.current[qIdx] === undefined) firstPickRef.current[qIdx] = label
    const key = turnIdx
    ...
    if (phase !== 'review' && (INST_SCRIPT_ONLY[instructor] || scriptWillAnswerWrong()
      /* 대본이 오답 경로를 통째로 갖고 있으면 그쪽에 맡긴다 (09-04 — 위 scriptBranchesWrong) */
      || (!ok && scriptBranchesWrong(qIdx)))) { goNext(); return }
    ...
    /* ── 학생 풀이 단계는 **앱이 먼저 다시 고르라고 하지 않는다** ──
       ⚠️ 2026-09-04 로 "강사 공통" 이 아니게 됐다. 윤다은 정오답분기 대본은 틀린 학생을
          오답 경로로 데려가 **대본이 정한 자리에서** 답을 다시 고르게 한다(gate ifWrong 의
          pickAnswer 턴). 그런 대본에서는 위에서 이미 비켜섰으므로 여기까지 오지 않는다.
          여기 남은 것은 **다시 풀리는 자리가 대본에 없는** 경우다 — 그때는 예전대로 한 번
          고른 것으로 끝낸다. 실제 시험과 같은 자리라 무르는 것이 기본값이 되면 안 된다.
  ...
  const answerSubjective = (text: string): boolean => {
    ...
    void (async () => {
      ...
      if (v === 'N' && (triesRef.current.get(turnIdx) ?? 0) < 1) {
        ...
        /* ── 말하고 **세워 두지 않는다** (09-18) ──
           "무슨 말인지 잘 모르겠어요. 수업 이어갈게요." 로 끝내 놓고 정작 아무것도 이어지지
           않아서, 학생이 무엇을 해야 하는지 모른 채 화면이 멈춰 있었다(사용자 지적).
           이 자리는 학생이 **아직 답을 안 한** 자리라 넘어갈 수는 없다 — 대신 무엇을 묻고
           있었는지 다시 말해 준다. 그래야 "이어간다" 는 말이 거짓말이 되지 않는다. */
        if (it.prompt) await say(`자, 다시 볼게요. ${it.prompt}`)
        return
      }
  ...
  /** 이 턴의 답을 뜻으로 판정 — 기대 답이 없는 자리는 무엇을 말해도 받아준다 */
  const judgeSubjective = async (said: string, it: { hint?: string; accepts?: string[] }): Promise<Verdict> => {
    const expected = [it.hint, ...(it.accepts ?? [])].filter(Boolean).join(' / ')
    if (!expected) return 'O'
    return askJudge(stripAudioTags(turn.tutor), expected, said)
  }
...
`
              : `[지금 단계] ${turn.stage}
[방금 강사가 한 말] ${stripAudioTags(turn.tutor)}
...
`
  ...
  const pickAskOption = (c: { label: string | null; text: string }) => {
    ...
    prevOkRef.current = null
    setTurnIdx(seekPlayable(turnIdx + 1))
  }
  ...
  const matchTypedChoice = (text: string, choices: { text: string }[]): number => {
    ...
    /* 번호로 답하는 꼴 — "1", "1번", "①", "1번이요" */
    const bare = stripTail(t)
    const n = /^([1-9])번?$/.exec(bare)?.[1] ?? (bare === '①' ? '1' : bare === '②' ? '2' : null)
    if (n && Number(n) <= choices.length) return Number(n) - 1
    /* O/X 는 기호로도 말로도 온다 */
    const ox = /^(o|ㅇ|네|응|맞아|맞아요|예|맞습니다|그래요)$/.test(bare) || /^(o|ㅇ|네|응|맞아|맞아요|예)$/.test(t) ? 'O'
      : /^(x|ㅌ|아니|아니요|아니에요|아닙니다|틀려요|틀렸어요)$/.test(bare) || /^(x|ㅌ|아니|아니요|아니에요)$/.test(t) ? 'X' : null
    if (ox) {
    ...
    const hits = choices
      ...
      .filter(({ c }) => c && (c === t || c === bare || c.includes(bare) || t.includes(c)))
    return hits.length === 1 ? hits[0].i : -1
  }
  ...
  const askAside = async (question: string, alreadyLogged = false) => {
    ...
    if (!t) return
    if (needsAnswer(t)) {
      /* ── 같은 물음을 **두 번 다시 읽지 않는다** (09-18) ──
         답이 규칙에 안 걸려 질문으로 새면, 강사가 설명하고 → 물음을 다시 읽고 → 학생이 또 답하고
         → 또 질문으로 새는 고리가 생긴다. 실제로 "기대어 두다" 라고 맞게 답한 학생이 이 고리에
         갇혔다(실측). 한 번 다시 읽어 준 자리는 그걸로 됐다 — 두 번째부터는 조용히 기다린다. */
      if (reaskedRef.current.has(at)) return
      reaskedRef.current.add(at)
  ...
  /** 물음을 다시 읽어 준 턴 — 한 턴에 한 번만(askAside 의 고리 방지) */
  const reaskedRef = useRef<Set<number>>(new Set())
  ...
  const st: ContentState = {
    ...
    /* ⚠️ 아직 한 번 더 풀릴 문항이면 **무조건 감춘다** — 채점 턴이든 이미 연 문항이든
       상관없다(willAskAgain). 다시 고르라고 해놓고 정답을 보여주면 고를 것이 없다. */
    hideVerdict: phase !== 'review'
      && (willAskAgain(turn.focusQ)
  ...
  useEffect(() => {
    ...
    setOpenAllQ((prev) => {
      let next = prev
      answeredQ.forEach((q) => {
        /* ── 다시 풀릴 문항은 **아직 열지 않는다** (09-04, 정답 색과 같은 이유) ──
           이 열기는 "학생이 다 풀고 나서 강사 설명을 듣는 동안" 을 위한 것이다(시트 지정).
           그런데 정오답분기 대본은 틀리면 한 번 더 풀린다 — 그 사이에 보기 넷을 다 열면
           다시 고를 때 문장이 전부 보인 채라 고르는 의미가 없다(실측 보고 09-04).
           재선택이 끝나면 이 효과가 다시 돌아 그때 연다(의존성의 turnIdx). */
        if (willAskAgain(q)) return
        if (!next.has(q)) { next = new Set(next); next.add(q) }
      })
      return next
    })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [answeredQ, cuePlaying, instructor, turnIdx])

  /** 말로 한 답이 이 턴의 **보기 중 하나인가** — 맞으면 그걸로 답하고 true.
   *
   *  음성 모드에서 선택지·O/X·정답 고르기 턴은 마이크가 닫혀 있었다. 태블릿을 세워 두고
   *  말로 수업하던 학생이 보기가 뜨는 순간 손을 뻗어 탭해야 했다 — 답하는 방식이 턴마다
   *  바뀌는 것은 앱의 사정이지 학생의 사정이 아니다(입력 모드에서 말할 수 있는 것과 같은 이유).
   *  버튼은 그대로 둔다. 눌러도 되고 말해도 된다.
   *  못 알아들으면 false 를 돌려주고, 부르는 쪽이 예전처럼 질문으로 흘려보낸다. */
  const answerByVoice = (text: string): boolean => {
    const it = turn.interaction
    /* 말로 들어온 것은 **전부 한 줄 남긴다** — 어디서 답으로 잡혔는지(규칙/판정기/되묻기)가
       한눈에 보여야 "말했는데 반응이 없다" 를 쫓을 수 있다. */
    console.log('[voice] 들은 말:', JSON.stringify(text), '· 이 턴:', it.kind)
    if (it.kind === 'choice' && choicePicked === null) {
      const at = matchTypedChoice(text, it.choices)
      if (at < 0) return false
      setChoicePicked(at); pickChoice(it.choices[at]); return true
    }
  ...
  /** 말한 것이 **어느 보기인가** — 화면 규칙(번호·라벨·글자)으로 못 고른 말만 여기로 온다.
   *  `>=0` 그 번호 · `-2` 질문이었다 · `-1` 모르겠다.
   *
   *  적어 둔 낱말만 맞추면 적어 두지 않은 말은 영영 못 알아듣는다(사용자 지적 09-18).
   *  음성인식이 깨져 오든, 보기를 자기 말로 바꿔 말하든, 판단은 **판정기가** 한다. */
  const judgeChoice = async (said: string, choices: string[]): Promise<number> => {
    ...
    try {
      const res = await fetch('/api/gemini', {
        ...
        body: JSON.stringify({
          pick: true,
          message: [
            `[강사 질문] ${stripAudioTags(turn.tutor)}`,
            ...choices.map((c, i) => `[보기 ${i + 1}] ${c}`),
            `[학생 말] ${said}`,
            fixSttJargon(said) !== said ? `[소리 보정 추정] ${fixSttJargon(said)}` : '',
          ].filter(Boolean).join('\n'),
        }),
      })
      if (!res.ok) { console.warn('[pick] 서버가 실패했다', res.status); return -1 }
      const v = String((await res.json()).dialogue ?? '').trim().toUpperCase()
      console.log('[pick] 학생 말:', JSON.stringify(said), '· 보정:', JSON.stringify(fixSttJargon(said)),
        '· 보기:', choices, '→', v)
      if (v.startsWith('Q')) return -2
      const n = Number(v.replace(/[^0-9]/g, ''))
      return Number.isInteger(n) && n >= 1 && n <= choices.length ? n - 1 : -1
    } catch (e) { console.warn('[pick] 판정 실패', e); return -1 } finally { clearTimeout(timer) }
  }

  /** 보기가 떠 있는데 **어느 보기인지 못 알아들었을 때**.
   *
   *  예전에는 이 말이 자유 질문으로 새서, 대본의 정오답 분기 대신 LLM 이 답을 해 버렸다
   *  (사용자 지적 09-18). 보기가 떠 있는 자리에서 학생이 하는 말은 **답이다** — 못 알아들었으면
   *  질문으로 넘길 것이 아니라 한 번 되묻고, 그래도 못 알아들으면 짚어 주고 넘어간다.
   *  (입력 모드의 되묻기·포기 갈래와 같은 규칙이다.) */
  const missedChoice = async (text: string) => {
    setChatLog((prev) => [...prev, { role: 'user', text }])
    const it = turn.interaction
    const tried = (triesRef.current.get(turnIdx) ?? 0) >= 1
    if (it.kind === 'choice' && (isGiveUp(text) || tried)) {
      logResponse(text, false)
    ...
    logResponse(text, null)
    await waitForCue()
    await say(it.kind === 'pickAnswer'
      ? '음, 잘 못 들었어요. 에이·비·씨·디 중에 골라 말해 줄래요?'
      : '음, 잘 못 들었어요. 보기 중에서 골라 볼래요?')
  }

  /** 규칙으로 못 고른 말을 **판정기에게 물어 본다.** 그래도 모르면 그때 되묻는다. */
  const pickByJudge = async (text: string) => {
    const it = turn.interaction
    const list = it.kind === 'choice' ? it.choices.map((c) => c.text)
      : it.kind === 'askOption' ? askableOf(turn).map((c) => c.text)
        : []
    const at = await judgeChoice(text, list)
    /* 그 사이 화면이 옮겨갔으면 둘 것이 없다 — 판정은 몇 초 걸린다 */
    if (turnsRef.current[turnIdxRef.current] !== turn) return
  ...
  /** 이 턴을 **말로 답할 수 있는가** — 주관식만이 아니라 보기 고르기도 포함한다(09-17).
   *  강사가 말하는 중·음원이 나가는 중에는 어차피 위에서 닫는다. */
  const voiceAnswerable = (() => {
    const it = turn.interaction
    if (it.kind === 'subjective') return !subjSent
    if (it.kind === 'choice') return choicePicked === null
    if (it.kind === 'askOption') return true
    /* ── **문제를 푸는 자리는 말로 받지 않는다** (사용자 지시 09-18) ──
       `pickAnswer` 는 시험 문항의 보기(A~D)를 고르는 단계다. 실제 시험에서 하는 동작이
       **고르는 것** 하나이기도 하고, 여기서 음성을 받으면 "에이"·"비" 가 잘못 전사될 때마다
       채점이 흔들린다 — 스캐폴딩 질문(choice·subjective)과 달리 여기는 **점수가 남는 자리**다.
       그래서 이 턴에서는 마이크를 아예 열지 않는다. */
    return false
  })()
  /** 이 턴에 **누를 것이 떠 있는가** — 손가락 힌트를 띄울지 가른다.
   *  문제 풀이(pickAnswer)는 빼 둔다: 거기는 누르는 것이 유일한 길이라 "말해도 돼요" 가 거짓말이고,
   *  안내는 지문 위 배너가 따로 한다. */
  const tapHintKind = (() => {
    const it = turn.interaction
    if (it.kind === 'choice') return choicePicked === null
  ...
  const scriptedVoice = useScriptedVoice(!!scripted && voicePhase && micVoiceMode, voiceOn, (text) => {
    ...
    /* 보기가 떠 있는 턴이면 **말한 것이 곧 고른 것이다** */
    else if (answerByVoice(text)) { /* 처리됨 — 대본의 정오답 분기가 이어서 돈다 */ }
    else if (turn.interaction.kind === 'subjective') answerSubjective(text)
    /* 맞장구·잡음은 흘린다. 보기 턴은 마이크가 오래 열려 있어서, 그냥 넘기면 주변 소리에
       강사가 묻지도 않은 설명을 시작한다(입력 모드 문지기와 같은 이유). */
    else if (isFiller(text) || isGibberish(text)) { /* 흘린다 */ }
    /* ── 보기가 떠 있으면 **답이 먼저다** ──
       뚜렷한 질문("무슨 뜻이에요?")만 질문으로 넘기고, 나머지는 못 알아들은 답으로 본다.
       이 갈래가 없을 때는 "비인 것 같아요" 같은 말이 전부 LLM 자유 질문으로 새어
       대본의 정오답 분기가 아예 돌지 않았다(09-18). */
    else if (tapHintKind && !looksLikeQuestion(text)) void pickByJudge(text)
    else void askAside(text)
  })
  ...
  const chatMessages: ChatMsg[] = (agentConnected || scripted) && chatLog.length
    ? revealLast(chatLog)
    : [
      { role: 'ai' as const, text: stripAudioTags(turn.tutor) },
      ...
      <PracticeStage
        ...
        onDone={(score) => {
          ...
          if (scriptedReview?.length && score.correct < score.total) {
            ...
            resetAsk()      // 실전 문항은 수업 문항과 다르다 — 들은 해설 기록을 비운다
            firstPickRef.current = {}   // 문항 번호가 수업과 겹친다 — 안 비우면 남의 답으로 길을 고른다
          ...
          if (score.correct < score.total) {
            ...
            firstPickRef.current = {}   // 위와 같은 이유 — 실전 문항 번호가 수업 것과 겹친다
            audioDoneRef.current = new Set(); respondedRef.current = new Set()
  ...
  const navStrip = !stripNav ? null : (() => {
            ...
            const nav = nextItemAt !== null
              ...
              : phase === 'review'
                ...
                : !freePlay
                  ? { label: '실전 문제 풀기 →', can: itemDone, hint: '마지막 문제의 단계를 마치면 열려요',
                      go: () => { stopVoice(); setPhase('practice') } }
      ...
      <div ref={splitRef} className={`flex-1 flex min-h-0 bg-white ${
        ...
        <div
          ...
          <div ref={contentRef} className={`flex-1 min-h-0 px-3 md:px-6 py-4 ${
            ...
            {/* ── 개념 학습은 **쪽지가 아니라 판**이다 ──
                구간 내내 화면을 통째로 갖는다. 마지막 턴까지 판이 서 있다가, 다음으로 넘어가는
                순간 아래 TipCard 가 그 내용을 들고 위 버튼으로 날아간다(tipExit). */}
            {(boardOpen || boardFlying) && scriptedConceptTip && !tipsHidden && (
              <ConceptBoard tip={scriptedConceptTip} flying={boardFlying}
        ...
        <TutorDock
          ...
          /* 파형은 **실제로 나가는 소리**를 따라간다. 에이전트가 붙어 있으면 그쪽 출력,
             대본 수업(mp3)이면 tts 쪽에서 물어 둔 분석기 값. 둘 다 없으면 undefined —
             그때만 화면이 기본 맥동으로 흔든다. */
          getTutorFreq={() => {
            try { return conversation.getOutputByteFrequencyData?.() ?? tutorVoiceFreq() }
            catch { return tutorVoiceFreq() }
          }}
          ...
          /* '질문 있어요' 버튼도, '강사가 답하는 중' 줄도 두지 않는다(09-18) — 강사가 곧
             말하기 시작하고 아바타가 그때 움직이므로, 아래에 노란 띠를 하나 더 걸 이유가 없다. */
                 ...
                 이걸 질문으로 넘기면 강사가 대화만 이어가고 대본으로 못 돌아온다(실측). */
              {
                ...
                if (it.kind === 'choice' && choicePicked === null) {
                  ...
                  /* ── 맞장구는 **그냥 흘린다** (09-08) ──
                     ...
                     (예전에는 학생 말이 쌓이면 보기가 위로 밀려 사라진 것처럼 보였다 —
                      선택지가 고정 영역으로 나오면서 그 문제는 없어졌다) */
                  if (isFiller(t)) {
                    setInputText('')
                    setChatLog((prev) => [...prev, { role: 'user', text: t, aside: true }])
                    /* 보기가 밀려 올라갈 걱정은 이제 없다 — 09-17 개편으로 선택지는 대화 흐름
                       밖 **고정 영역**에 산다. 대화가 쌓여도 그 자리에서 움직이지 않는다.
                       (예전에는 카드가 채팅 한 칸이라, 학생 말이 쌓이면 화면 밖으로 밀렸다) */
                    return
                  }
                  ...
                  if (isGibberish(t)) {
                    ...
                    /* ⚠️ 카드 자리 비우기는 **말하기 전**이다. `say` 는 말풍선을 먼저 넣고
                       (09-17 개편 전에는 여기서 카드 자리를 다시 잡아 줘야 했다 — 선택지가
                       고정 영역으로 나오면서 그 손질이 통째로 필요 없어졌다) */
                    void (async () => {
                      await waitForCue()
                  ...
                  /* ── 규칙으로 못 골랐으면 **판정기에게 묻는다** (09-18) ──
                     여기서 아래 '답이 아니면 질문이다' 로 흘려보냈더니, 맞게 답한 학생의 말까지
                     자유 질문이 되어 강사가 설명만 하고 물음을 다시 읽는 고리에 갇혔다(실측:
                     "기대어 두다" 를 세 번 답했는데 계속 제자리). 음성 모드와 같은 길로 보낸다. */
                  setInputText('')
                  void pickByJudge(t)
                  return
                }
          ...
          overlay={tipSheet && !tipsHidden
            ...
            : undefined}
          /* 입력 모드에서도 **눌러서** 말할 수 있다 — 모드는 권하는 것이지 잠그는 게 아니다 */
          pushTalk={pushTalk} setPushTalk={setPushTalk}
...
export function PracticeStage({ lesson, onExit, onDone, onJumpPhase, nextLabel, steps, solvingHint, paperLook }: {
  ...
  const st: ContentState = {
    ...
    /* 해설은 **여기서만** 켠다 — 채점 뒤 정답 보기 아래(수업 화면은 강사가 말로 짚는다) */
    showWhy: graded,
    struck,
...
/* 전략 정리와 뜻 고르기(빈출 표현)를 가르던 자리였다 — 그쪽만 "틀려도 그대로 채점" 이었다.
   이제는 **둘 다** 말한 그대로 받고 강사가 짚을 때 채점하므로 가를 이유가 없다(메모 46·53행). */
...
/** loose 로 줄인 문자열 + 줄인 글자가 **원문 어디서 왔는지**의 표.
 *  찾은 자리를 원문으로 되짚어 **학생이 실제로 한 말**을 잘라내는 데 쓴다.
 *  ⚠️ 이게 없으면 정답 표기로 바꿔치기하게 된다 — "동작" 이라고 말했는데 칸에는
 *     "인물의 동작" 이 떴다(08-28 지적). 맞혔다고 남의 말로 갈아 끼우면 안 된다. */
const LOOSE_DROP = /[\s+·・.,()~]/
function looseIndexed(s: string): { hay: string; idx: number[] } {
  let hay = ''
  const idx: number[] = []
  for (let i = 0; i < s.length; i++) {
    if (LOOSE_DROP.test(s[i])) continue
    hay += s[i].toLowerCase()
    idx.push(i)
  }
  return { hay, idx }
...
/** 닻 조각의 길이 범위.
 *  · 최소 2 — "___ 확인" 처럼 **뒤 대본이 두 글자뿐인** 문항이 있다.
 *  · 다만 **긴 대본을 두 글자까지 줄여 찾지는 않는다**(PROBE_MIN). 14글자짜리 대본의 꼬리를
 *    "이면" 까지 줄이면 문장 뒤쪽의 "대상이면" 에 걸려 자리가 통째로 어긋난다(실측). */
const ANCHOR_MIN = 2
const ANCHOR_MAX = 14
const PROBE_MIN = 4
/** 빈칸 하나에 들어갈 만한 최대 길이 — 이보다 길게 잘렸으면 닻을 잘못 잡은 것이다 */
const SLOT_MAX = 40

/** 대본 조각을 학생 말에서 찾는다. 통째로 안 걸리면 **짧게 줄여 가며** 다시 본다 —
 *  인식이 조사나 한두 글자를 흘려도 자리는 잡아야 한다.
 *  `side`: 'tail' 은 조각의 **끝**(빈칸 바로 앞), 'head' 는 조각의 **처음**(빈칸 바로 뒤). */
function matchAnchor(hay: string, from: number, part: string, side: 'tail' | 'head'): { at: number; len: number } | null {
  const clean = looseIndexed(part).hay
  if (clean.length < ANCHOR_MIN) return null
  /* 조각이 짧으면 통째로만 찾는다. 길면 PROBE_MIN 까지만 줄인다. */
  const floor = Math.min(clean.length, PROBE_MIN)
  for (let len = Math.min(clean.length, ANCHOR_MAX); len >= floor; len--) {
    const probe = side === 'tail' ? clean.slice(clean.length - len) : clean.slice(0, len)
    const at = hay.indexOf(probe, from)
    if (at >= 0) return { at, len }
  }
  return null
}

/** ── 학생이 읽은 문장을 **빈칸별로** 가른다 ──
 *  빈칸이 둘 이상이면 키워드 찾기만으로는 모자란다. 틀리게 말하면 키워드가 안 걸리고,
 *  그때 예전 코드는 **전체 전사를 빈 칸마다 똑같이 복사**했다 — 두 칸에 같은 말이 들어갔다
 *  (08-28 지적). 답이 아니라 **대본의 고정된 글자**를 닻으로 잡으면 틀리게 말해도 자리는 잡힌다:
 *
 *    대본  "주어가 …주체이면 ___ , 주어가 …대상이면 ___를 쓴다."
 *    학생  "주어가 …주체이면 [능동태] 주어가 …대상이면 [수동태]를 쓴다"
 *                            └ 1번 칸                  └ 2번 칸
 *
 *  못 자른 칸은 null 이다 — 아무 말이나 채워 넣지 않는다. */
function splitByTemplate(en: string, said: string): (string | null)[] {
  const parts = en.split('___')
  const n = parts.length - 1
  if (n < 1) return []
  const { hay, idx } = looseIndexed(said)
  const out: (string | null)[] = []
  let cur = 0
  for (let i = 0; i < n; i++) {
    const before = matchAnchor(hay, cur, parts[i], 'tail')
    const start = before ? before.at + before.len : cur
    const after = matchAnchor(hay, start, parts[i + 1], 'head')
    /* ⚠️ **앞 대본이 있는데 못 찾았으면 자리를 못 믿는다.** 그냥 이어 붙이면 그 칸이 앞의 말을
       통째로 삼킨다 — 인식이 조사 한 글자를 흘리기만 해도("주체이면"→"주체면") 첫 칸에
       문장 절반이 들어갔다(실측). 그럴 땐 비워서 돌려보내고 키워드 찾기에 맡긴다. */
    if (!before && looseIndexed(parts[i]).hay.length >= ANCHOR_MIN) {
      out.push(null)
      if (after) cur = after.at        // 뒤 닻이라도 잡혔으면 거기서부터 다음 칸을 본다
      continue
    }
    /* 마지막 칸 뒤에 대본 글자가 없으면(문장이 빈칸으로 끝남) 남은 말이 통째로 그 칸이다 */
    const end = after ? after.at : hay.length
    if (end <= start) { out.push(null); continue }
    const slot = said.slice(idx[start], idx[end - 1] + 1).trim()
    /* 한 칸에 들어갈 말치고 지나치게 길면 닻을 잘못 잡은 것이다 — 넣지 않고 돌려보낸다 */
    out.push(slot && slot.length <= SLOT_MAX ? slot : null)
    cur = end
  }
  return out
}
...
/** ── 대본 문장을 **어디까지 읽었나** ──
 *  낱말을 앞에서부터 차례로 들린 말에 맞춰 보고, **가장 멀리 맞은 낱말까지**의 길이를 돌려준다.
 *  ⚠️ 못 맞은 낱말에서 **멈추지 않는다.** 멈추게 했더니 두 군데서 걸렸다(08-28 실측):
 *     ① **빈칸(`___`)** — 학생이 그 자리에 넣은 말은 대본에 없으니 영영 못 찾는다. 그래서
 *        빈칸 뒤 글자가 끝까지 안 진해졌다. 빈칸은 찾지 말고 지나가야 한다.
 *     ② 인식이 낱말 하나를 흘리면 그 뒤가 통째로 흐린 채 남는다.
 *     둘 다 "건너뛰고 계속" 으로 풀린다 — 어차피 **앞으로만** 찾으므로 순서는 지켜진다.
 *  ⚠️ 이건 **보여주기 전용**이다 — 채점에는 쓰지 않는다(브라우저 내장 인식이라 못 믿는다). */
const BLANK_WORD = /^_+$/
function spokenWordCount(words: string[], said: string): number {
  const hay = looseIndexed(said).hay
  let cur = 0
  let last = -1
  for (let i = 0; i < words.length; i++) {
    const w = words[i]
    if (BLANK_WORD.test(w)) continue       // 빈칸 — 학생이 넣은 말은 대본에 없다
    const lw = looseIndexed(w).hay
    if (!lw) continue                      // 공백·기호
    const at = hay.indexOf(lw, cur)
    if (at < 0) continue                   // 흘려들은 낱말 하나 때문에 뒤를 다 끄지 않는다
    cur = at + lw.length
    last = i
  }
  return last + 1
...
function RecapBlankSentence({ text, filled, corrects, answers, graded, live, onFill }: {
  text: string
  /** 빈칸을 **그 자리에서** 받는다 — 없으면 예전처럼 읽기 전용 칸이다 */
  onFill?: (blank: number, said: string) => void
  ...
  /** 칸마다 정답 */
  answers: string[]
  /** **강사가 이 문항을 짚기 시작했나** — 채점 색은 그때 켠다 (구현 중 메모 46행) */
  graded?: boolean
  /** 지금 **말하는 중에** 들리는 말. 있으면 문장을 흐리게 깔고 읽은 데까지 진하게 켠다.
   *  없으면(undefined) 평소대로 그린다. */
  live?: string
}) {
  /* ── 무엇을 빈칸에 넣을 것인가 ──
     **언제나 학생이 한 말 그대로다.** 맞았다고 시트의 정답 표기로 갈아 끼우지 않는다 —
     "동작" 이라고 말했는데 칸에 "인물의 동작" 이 뜨면, 학생은 자기가 안 한 말을 자기 답으로
     읽게 된다(08-28 지적).
  ...
  let blankNo = -1
  /* 타이핑 중인 글자 — 확정(Enter·포커스 떠남) 전까지만 여기 있고, 확정되면 지운다.
     지운 뒤에는 filled(진짜 답)로 되돌아가므로 두 곳에 같은 값이 남지 않는다. */
  const [drafts, setDrafts] = useState<Record<number, string>>({})

  /* ── 읽어가는 자리 비추기 ──
     **말하는 동안에만 흐려진다** (09-21). 예전에는 안 채운 문장이 처음부터 흐렸는데, 아직
     아무것도 안 한 화면이 회색으로 깔려 있어 읽을 것이 없는 자리처럼 보였다(사용자 지적).
     이제 평소에는 평소 색이고, 마이크를 눌러 읽기 시작하면 그때 문장이 회색으로 내려앉아
     **인식된 데까지만** 진해진다 — "지금 내 말이 닿고 있나" 를 글자로 확인하는 자리다.
     Scribe 는 중간 결과가 없어서 이 표시는 브라우저 내장 인식이 만든다 — 못 받으면 아예
     흐려지지 않을 뿐, 답은 그대로 들어간다. */
  const reading = live !== undefined
  ...
  const blankOf = (i: number) => {
    ...
    /* ── 빈칸을 **누르면 바로 적는다** (09-21) ──
       예전에는 문장 아래 입력칸이 따로 있어서, 화면의 빈칸은 눌러도 아무 일이 없었다 —
       빈칸처럼 생긴 것을 누르는 게 먼저인데 거기서 한 번 막혔다(사용자 지적).
       이제 칸 자체가 입력칸이다. 말로 채우는 길(마이크)은 그대로 남는다. */
    if (onFill && !graded) {
      const v = drafts[i] ?? mine ?? ''
      const commit = () => {
        const t = v.trim()
        setDrafts((d) => { const n = { ...d }; delete n[i]; return n })
        if (t && t !== mine) onFill(i, t)
      }
      return (
        <input value={v}
          ...
          onBlur={commit}
          aria-label={`${i + 1}번째 빈칸에 들어갈 말`}
          style={{ width: `${Math.max(7, v.length + 2)}ch` }}
          className={`inline-block mx-1 px-2 py-0.5 text-center align-baseline font-black text-[14px]
                      border-b-2 outline-none transition-colors placeholder:font-semibold placeholder:text-[#B4BCC8]
                      focus:border-[#2563EB] focus:bg-[#EFF6FF] ${
            mine === undefined ? 'border-[#CBD5E1] bg-[#F8FAFC] text-[#1C1B33]'
  ...
  return (
    /* 문장은 **굵게 쓰지 않는다**(09-21 사용자 지시) — 굵은 것은 내가 채운 빈칸뿐이라야
       어디가 내 답인지 한눈에 갈린다. 본문까지 굵으면 문장 전체가 강조로 읽힌다. */
...
/** ── 어휘 한 줄 — **표 한 장**으로 본다 (09-21) ──
 *  빈출 표현 정리는 "표현 하나 ↔ 뜻 고르기" 열 번이다. 그런데 전략 정리용 부품(빈칸 렌더러·
 *  마이크·읽어가는 자리 비추기)을 그대로 쓰고 있어서, 문항마다 장치가 셋씩 붙고 10문항이
 *  서너 화면으로 늘어났다 — 한눈에 안 들어온다는 지적이 거기서 나왔다.
 *  카드를 줄로 낮추고, 남는 것은 **표현과 보기 셋**뿐이다.
 *  · 보기는 **3열 고정 그리드** — 칩이 줄마다 다르게 접히면 훑는 눈이 자꾸 걸린다
 *  · **짚는 줄 음영은 두지 않는다**(09-21). 어휘 장은 시트에 문항별 피드백이 없어 실제로
 *    짚고 가지 않는데, 마무리 한마디가 1번에 걸려 첫 줄만 잠깐 켜졌다 꺼졌다(사용자 지적).
 *  · 손가락 목표 44px 은 지킨다(태블릿) */
function RecapVocabRow({ index, sentence, picked, correct, graded, onPick }: {
  ...
  /* 시트가 "rinse = ___" 로 적어 둔다 — 빈칸은 오른쪽 보기가 대신하므로 표현만 남긴다 */
  const term = sentence.en.replace(/\s*=\s*_+\s*$/, '')
  return (
    <div className="grid grid-cols-[22px_minmax(0,150px)_minmax(0,1fr)] gap-3 items-start px-3 py-2.5">
      ...
      <div className="pt-2 flex items-center gap-1.5 min-w-0">
        ...
        <span className="text-[15.5px] font-extrabold text-[#1C1B33] leading-snug break-words">{term}</span>
      </div>
      {/* ── 채점이 끝나면 **단어장 한 줄로 접는다** (09-21) ──
          방해 보기 둘은 고르는 동안에만 쓸모가 있다. 채점 뒤에도 남겨 두면 열 줄 × 세 칸이
          그대로 남아 정답만 훑을 수가 없다. 그렇다고 **정답을 첫 칸으로 옮기지는 않는다** —
          자리가 움직이면 "내가 가운데를 눌렀다" 는 기억이 깨지고 화면이 한 번 흔들린다.
...
function RecapCard({ index, sentence, filled, corrects, onPick, onSpeak, onInterim, onStart, live, active, graded }: {
  index: number; sentence: RecapSentence
  ...
  /** 말하는 중에 들리는 말을 위로 올린다 (읽어가는 자리 표시용) */
  onInterim?: (t: string) => void
  /** 마이크를 눌러 **다시 말하기 시작**했다 — 이 문항의 답을 비운다 */
  onStart?: () => void
  /** 지금 이 문항을 읽는 중이면 그 말 — 아니면 undefined */
  live?: string
  ...
  const done = at === -1
  /** ── 다 채운 뒤에도 **다시 말할 수 있다** ──
   *  예전에는 칸이 다 차면 마이크를 감췄다. 그래서 잘못 들어간 말을 고칠 방법이 없었다
   *  (08-28 지적). 이제 **강사가 이 문항을 짚기 전까지는** 몇 번이고 다시 말할 수 있다 —
   *  다시 말하면 그 말로 덮어쓴다. 강사가 짚기 시작하면(graded) 채점이 끝난 자리라 닫는다.
   *  보기를 고르는 문항(어휘)도 09-21 부터 같다 — 잘못 고른 것을 채점 전까지 바꿀 수 있다. */
  const canSpeak = !graded
  ...
  return (
    ...
    <div className={` border bg-white p-4 transition-all ${
      ...
      <div className="flex items-start gap-2.5 mb-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <RecapBlankSentence text={sentence.en} filled={filled} corrects={corrects} answers={answers}
            graded={graded} live={live}
            onFill={sentence.choices.length ? undefined : (b, said) => onPick(said, b)} />
        </div>
        {/* ── 마이크는 **문장 옆**이다 ──
            아래 줄에 두면 문항마다 버튼과 안내가 한 줄씩 더 쌓여, 문장보다 조작이 더 커 보인다.
            말할 대상(문장) 바로 옆에 두면 무엇을 읽어야 하는지가 버튼이 가리켜 준다.
      ...
      {/* ── 보기가 없으면 **말하거나, 빈칸에 바로 적는다** ──
          입력칸을 문장 아래 따로 두던 자리였다(RecapAnswerInput). 이제 문장의 빈칸 자체가
          입력칸이라 여기서는 지웠다 — 같은 일을 하는 칸이 두 개면 어느 쪽이 진짜인지 모른다.
          말로 하면 빈칸을 채운 **문장 전체**를 읽고 한 번의 말에서 칸을 순서대로 채운다
          (fillFromSpeech). 뜻 고르기(어휘)는 그대로 보기다. */}
      ...
      <div className="flex flex-wrap items-center gap-2 pl-9">
        {sentence.choices.map((c) => {
          ...
          return (
            /* ── 채점 전에는 **몇 번이고 바꿀 수 있다** (09-21) ──
               예전에는 하나 고르면 그 자리에서 잠겼다. 잘못 누르면 고칠 길이 없어서, 틀린 줄
               알면서 강사가 짚어 줄 때까지 기다려야 했다(사용자 지적). 말로 답하는 자리는
               이미 다시 말해 덮어쓸 수 있다 — 보기도 같은 규칙으로 맞춘다.
               잠기는 때는 **강사가 이 문항을 짚기 시작할 때**(graded) 하나뿐이다. */
            <button key={c} disabled={graded} onClick={() => onPick(c, 0)}
              className={`text-[12px] font-semibold border px-3 py-1.5 transition-colors ${
                  ...
                  /* 아직 고르는 중 — 내가 고른 것만 파랗게 남고, 나머지도 그대로 누를 수 있다.
                     정답은 아직 말하지 않는다. */
                  : filled?.[0] === c ? 'border-[#2563EB] bg-[#EFF6FF] text-[#1D4ED8]'
...
function WrapStage({ lesson, practiceScore, teacherName, teacherImg, instructor, onExit, onDone, onJumpPhase, scriptedSummary }: {
  ...
  /** 칸마다 맞았는가 — **넣을 때 정해서 들고 있는다.** 화면에 남는 글자로 되풀이 채점하지
   *  않는다: 칸에는 학생이 한 말이 그대로 들어가는데, 그 말을 다시 채점기에 넣으면 판정이
   *  글자에 따라 흔들린다(맞혔는데 잘라낸 조각이 키워드와 안 겹치는 일이 있다). */
  const [verdicts, setVerdicts] = useState<Record<string, (boolean | undefined)[]>>({})
  /** 지금 판정기에 물어 둔 칸이 **몇 개인가** — 0 이 될 때까지 강사가 짚기 시작하지 않는다.
   *  먼저 시작하면 곧 초록으로 바뀔 칸을 빨갛게 둔 채 해설이 지나간다. */
  const [judging, setJudging] = useState(0)
  /** 지금 **소리 내어 읽는 중인** 문항과 거기서 들린 말 — 문장을 따라 밝히는 데만 쓴다 */
  ...
  /** 채운 답이 맞았는가 — 아직 안 채운 칸은 undefined 다(화면이 '덜 찼음' 을 그걸로 안다) */
  const correctsOf = (s: RecapSentence): (boolean | undefined)[] =>
    blanksOf(s).map((_, i) => (fills[s.id]?.[i] === undefined ? undefined : (verdicts[s.id]?.[i] ?? false)))
  ...
  const FILL_HINT = items[0]?.choices.length
    ? '빈칸에 들어갈 말을 골라 보세요. 다 골랐으면 아래 채점하기를 눌러 주세요.'
    : '빈칸에 들어갈 말을 직접 말하거나 글로 입력해 보세요. 다 채웠으면 아래 채점하기를 눌러 주세요.'
  ...
  const runWrapUp = async () => {
    ...
    setPlaying(true)
    /* ── 채점은 **한 번에** 켠다 (09-21) ──
       예전에는 강사가 짚는 문항만 하나씩 색이 들어왔다(메모 46행). 그러면 10문항짜리 장에서
       내가 몇 개를 맞혔는지 해설이 다 끝날 때까지 알 수 없고, 아직 회색인 칸이 채점을 안 한
       것인지 기다리는 것인지도 구분이 안 됐다. 이제 다 채우는 순간 전부 채점되고,
       강사가 짚는 자리는 **활성 표시(active)** 로만 알린다 — 채점과 진행을 갈라 둔다. */
    setRevealed(new Set(items.map((s) => s.id)))
  ...
  /* ── 강사가 짚는 문항으로 **따라 내려간다** (09-21) ──
     채점하기를 누르면 강사가 1번부터 하나씩 짚는데, 목록이 길면 짚는 문항이 화면 밖에 있다.
     말은 들리는데 어디를 말하는지 안 보이면 활성 표시(파란 링)가 아무 일도 못 한다.
     가운데로 당겨 두면 앞뒤 문항도 같이 보인다. */
  ...
  /* ── 채점은 **학생이 누를 때** 시작한다 (09-21) ──
     예전에는 마지막 칸이 채워지는 순간 저절로 돌았다. 그래서 마지막 문항만은 잘못 눌러도
     고칠 틈이 없었다(사용자 지적) — 채우는 행위가 곧 제출이었기 때문이다. 이제 다 채우면
     아래 '채점하기' 가 살아나고, 그때까지는 어느 칸이든 몇 번이고 고칠 수 있다.
     ⚠️ 판정기에 물어 둔 칸이 남아 있으면(judging) 버튼을 잠근다 — 몇 초 뒤 초록이 될 칸을
        빨갛게 둔 채 해설을 시작하면, 학생은 맞힌 것을 틀렸다고 들은 셈이 된다. */
  const startedRef = useRef(-1)
  const startWrapUp = () => {
    if (!allDone || judging > 0 || startedRef.current === page) return
   ...
   *  ("핵심 요약 … 못 맞추면 아예 안넘어가짐", 메모 53행). 지금은 말한 그대로 넣고,
   *  맞았는지는 강사가 짚을 때 색으로 켠다. 틀린 것이 곧 정보이기도 하다.
   *  마지막 칸이 채워지면 강사가 알아서 짚기 시작한다. */
  /** ── 이 문항을 **처음 상태로 되돌린다** ── (마이크를 다시 누른 자리)
   *  칸을 비우고 판정도 함께 버린다. 판정만 남으면 채우지도 않은 칸이 채점된 것으로 보인다. */
  const clearItem = (id: string) => {
    setFills((p) => { const n = { ...p }; delete n[id]; return n })
    setVerdicts((p) => { const n = { ...p }; delete n[id]; return n })
    /* 아직 강사가 시작하지 않았으면 **시작 표시도 풀어 준다** — 안 그러면 다시 채워도
       "이 장은 이미 시작했다"로 남아 강사가 영영 시작하지 않는다. 이미 말하는 중이면
       그대로 둔다(그 표시가 두 번 시작하는 것을 막고 있다). */
    if (!playing) startedRef.current = -1
  }

  /** ── 낱말로 못 가린 답은 **판정기에 묻는다** (09-18) ──
   *  시트가 한 칸에 인정 답을 여럿 적어 두는 자리가 있고("주어와 동사의 의미 관계, 의미"),
   *  학생은 배운 말을 제 말로 바꿔 꺼낸다("의미 관계요" · "주어랑 동사 관계"). 글자 겹침만
   *  보면 그런 답이 오답으로 남는다 — 정리 화면은 받아쓰기가 아니라 **되짚는 자리**다.
   *  맞다고 나온 것만 뒤집는다. 판정이 실패하면 낱말 판정 그대로 둔다.
   *  ⚠️ 보기가 있는 자리(어휘 확인)는 묻지 않는다 — 고른 그대로 채점해야 무엇을 모르는지 보인다. */
  const confirmByLlm = (s: RecapSentence, said: string, blank: number) => {
    if (s.choices.length) return
    const b = blanksOf(s)[blank] ?? blanksOf(s)[0]
    const expected = Array.from(new Set([b.answer, ...b.keywords])).filter(Boolean).join(' / ')
    if (!expected) return
    setJudging((n) => n + 1)
    void (async () => {
      try {
        const v = await askJudge(`${s.head ? `${s.head} — ` : ''}${s.en.replace(/___/g, '____')}`, expected, said)
        if (v !== 'O') return
        setVerdicts((p) => {
          const cur = p[s.id] ? [...p[s.id]] : blanksOf(s).map(() => undefined as boolean | undefined)
          cur[blank] = true
          return { ...p, [s.id]: cur }
        })
      } finally { setJudging((n) => n - 1) }
    })()
  }

  const answerOne = (s: RecapSentence, said: string, blank = 0) => {
    if (!said.trim()) return
    const ok = isRight(s, said, blank)
    if (!ok) confirmByLlm(s, said, blank)
  ...
  const fillFromSpeech = (s: RecapSentence, said: string): { text: string | null; ok: boolean }[] => {
    const blanks = blanksOf(s)
    /* ① 대본의 고정 글자를 닻으로 칸을 가른다 — 틀리게 말해도 자리는 잡힌다 */
    const bySlot = splitByTemplate(s.en, said)
    /* ② 닻이 안 잡힌 칸만 키워드로 찾아 본다 (빈칸 하나짜리는 대개 이쪽으로 끝난다) */
    const { hay, idx } = looseIndexed(said)
    let from = 0
    return blanks.map((b, i) => {
      const slot = bySlot[i] ?? null
      if (slot) {
        /* 잘라낸 말 그대로 넣는다 — 맞았는지는 키워드로 따로 본다(시트 표기로 갈아 끼우지 않는다) */
        return { text: slot, ok: isRight(s, slot, i) }
      }
      ...
      if (!hit) return { text: null, ok: false }
      const start = idx[hit.at]
      const end = idx[Math.min(hit.at + hit.len, idx.length) - 1]
      from = hit.at + hit.len
      return { text: said.slice(start, end + 1).trim() || said.trim(), ok: true }
    })
  }
  ...
  const speakAnswer = (s: RecapSentence, transcript: string, blank = 0) => {
    ...
    if (found.length === 1 && !found[0].text && isRight(s, transcript, 0)) found[0] = { text: transcript.trim(), ok: true }

    /* ── 아직 아무것도 못 담은 칸을 어떻게 할 것인가 ──
       메모 46행은 "학생이 말한 거 그대로 입력" 이다. 그런데 **빈칸이 여럿일 때 전체 전사를
       칸마다 복사하면 같은 말이 두 번 들어간다**(08-28 지적). 그래서 통째로 넣는 것은
       **채울 칸이 하나 남았을 때뿐**이다. 여럿이면 비워 두고 다시 말하게 한다 —
       화면의 빈칸이 어디가 안 찼는지 이미 보여 준다. */
    const prev = fills[s.id]
    const blanks = blanksOf(s)
    const openIdx = blanks.map((_, i) => i).filter((i) => prev?.[i] === undefined && !found[i].text)
    const soleOpen = openIdx.length === 1 && blanks.length > 1 ? openIdx[0] : (blanks.length === 1 ? 0 : -1)
    const put = found.map((f, i) => (f.text ? f : i === soleOpen ? { text: transcript.trim(), ok: isRight(s, transcript, i) } : f))

    setFills((p) => {
      const cur = p[s.id] ? [...p[s.id]] : blanks.map(() => undefined as string | undefined)
      put.forEach((f, i) => { if (f.text) cur[i] = f.text })
      return { ...p, [s.id]: cur }
    })
    /* 판정은 **여기서 정해 못 박는다** — 화면 글자를 나중에 되풀이 채점하지 않는다 */
    setVerdicts((p) => {
      const cur = p[s.id] ? [...p[s.id]] : blanks.map(() => undefined as boolean | undefined)
      put.forEach((f, i) => { if (f.text) cur[i] = f.ok })
      return { ...p, [s.id]: cur }
    })
    /* 낱말로 못 가린 칸만 판정기에 묻는다 — 뜻으로 맞았으면 위 판정을 뒤집는다 */
    put.forEach((f, i) => { if (f.text && !f.ok) confirmByLlm(s, f.text, i) })
  }
    ...
    <div className="h-dvh flex flex-col bg-[#F5F8FE] overflow-hidden">
      ...
      <div ref={pageRef} className="flex-1 overflow-y-auto px-4 md:px-6 py-5">
        <div className="max-w-[640px] mx-auto space-y-4">
          ...
          {/* 장 제목 줄("… 핵심 정리 · 4문항 · 1/2")은 없앴다 (09-21 사용자 지시) —
              무엇을 하는 화면인지는 상단 표시줄과 강사 말이 이미 말한다. */}

          {/* ── 조작법은 **여기 한 번만** ──
              예전엔 문항마다 마이크 옆에 같은 문장이 붙어 있었다. 10문항짜리 장에서는 그 말이
              열 번 반복돼 화면이 지시문으로 뒤덮인다(08-28 지적).
              ⚠️ "카드 위에 안내 줄글을 두지 않는다"(강사가 위에서 말한다)는 앞선 결정이 있었는데,
                 여기만 되살린다 — Scribe 는 배치 전사라 **한 번 더 눌러 끝내야** 글자가 온다.
          ...
          {/* 어휘 장(보기가 있는 문항만 모인 장)은 **표 한 장**이다 — 전략 정리 장은 예전 카드 그대로 */}
          {items.every((s) => s.choices.length > 0) ? (
            <>
            {/* 채점이 켜지면 **몇 개 맞혔는지 먼저** 말해 준다 — 줄마다 색을 세게 하지 않는다 */}
            {items.some((s) => revealed.has(s.id)) && (() => {
              /* ⚠️ 여기서 **"강사가 하나씩 짚어 줄게요" 라고 하지 않는다**(09-21 지적).
                 어휘 장은 시트에 문항별 피드백이 없고 마무리 한마디만 있어서, 실제로는
                 하나씩 짚지 않는다 — 화면이 일어나지 않을 일을 예고하면 학생은 기다린다.
                 대신 표가 이미 말하고 있는 것(초록이 정답)만 짚어 준다. */
              const got = items.filter((s) => correctOf(s) === true).length
              return (
                ...
                <div className="flex items-center gap-2.5 rounded-xl border border-[#BFDBFE] bg-[#EFF6FF] px-4 py-2.5">
                  ...
                  <span className="text-[12.5px] font-bold text-[#3F5C86]">
                    {got === items.length
                      ? `${got}개 다 맞혔어요!`
                      : `${got}개 맞혔어요. 틀린 표현 다시 한번 확인해 보세요!`}
                  </span>
                </div>
              )
            })()}
            <div className="border border-[#E5E7EB] bg-white divide-y divide-[#F1F3F7]">
              {items.map((s, i) => (
                <RecapVocabRow key={s.id} index={i} sentence={s}
                  picked={fills[s.id]?.[0]} correct={correctOf(s) === true}
                  graded={revealed.has(s.id)}
                  onPick={(c) => pick(s, c, 0)} />
              ))}
            </div>
            </>
          ...
          <div className="space-y-3">
            {items.map((s, i) => (
              <div key={s.id} ref={(el) => { cardRefs.current[i] = el }}>
                <RecapCard index={i} sentence={s} filled={fills[s.id]}
                  corrects={correctsOf(s)} graded={revealed.has(s.id)}
                  live={live?.id === s.id ? live.text : undefined}
                  onInterim={(t) => setLive(t === '' ? null : { id: s.id, text: t })}
                  onStart={() => clearItem(s.id)}
                  active={activeIdx === i}
      ...
      {/* ── 버튼은 '채점하기' → '다음' 두 단계다 (09-21) ──
          마지막 칸을 채우면 강사가 알아서 시작하던 자리였다. 누를 것이 없어 좋았지만,
          **마지막 문항만 고칠 틈이 없었다** — 채우는 것이 곧 제출이라서다. 제출을 학생 손에
          되돌려 준다. 채점 전에는 '채점하기', 해설이 끝나면 '다음/완료하기'.
          ⚠️ **건너뛰기는 없앴다** (메모 51행, 콘텐츠 파트 답: "강사 말 얼마 안되니 끝까지
             듣고 완료하기 버튼 활성화되게"). 정리 멘트가 이 화면의 알맹이라, 건너뛰면
             남는 게 빈칸 채우기뿐이다. 말하는 동안에는 지금 무엇을 하는 중인지만 알린다. */}
      <div className="shrink-0 bg-white border-t border-[#EBEBF0] px-4 md:px-6 py-3">
        <div className="max-w-[640px] mx-auto flex items-center justify-end gap-3">
          {playing && <span className="text-[12px] font-bold text-[#94A3B8]">강사가 정리하는 중이에요</span>}
          {/* ── 채점하기 — **다 채운 뒤에야** 살아난다 ──
              이 버튼을 누르기 전까지는 고친 답이 그대로 반영된다. 누르면 전 문항이 한꺼번에
              채점되고 강사가 하나씩 짚기 시작한다(runWrapUp). 채점이 끝나면 사라진다. */}
          {!played && !playing && (
            <button disabled={!allDone || judging > 0} className={PRIMARY_BTN} onClick={startWrapUp}>
              {judging > 0 ? '답을 확인하는 중…' : '채점하기'}
            </button>
          )}
          {!lastPage ? (
  ...
  if (it.kind === 'choice') {
    ...
    if (isOX) {
      return (
        <div>
          ...
          {/* ── 고르고 나서 **글로 또 말하지 않는다**(09-18) ──
              맞았는지 틀렸는지는 이미 보기 색(초록/빨강)이 말하고, 왜 그런지는 **강사가 곧 말한다.**
              그 사이에 같은 말을 글로 한 번 더 얹으면 학생이 강사 말을 안 듣고 글을 읽는다. */}
        </div>
      )
    }
    ...
    return (
      <div>
        ...
        {/* 피드백 문구는 두지 않는다 — 위 O/X 갈래의 주석 참고 */}
      </div>
    )
  }
