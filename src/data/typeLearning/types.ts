...
/** ── 문항 하나가 끝날 때 화면에 뜨는 '토익 TIP' 카드 ──
 *
 *  시트에서 S7(표현 정리) **바로 다음 줄**에 단계 이름 없이 대사 칸에만 적혀 있다.
 *  강사가 읽는 말이 아니라 **학생이 눈으로 보는 카드**라 발화와 따로 담는다.
 *
 *  `body` 를 문단별로 쪼개지 않고 **줄 그대로** 들고 있는 이유: 시트가 자유롭게 쓴다 —
 *  소제목(`*상태를 나타내는 be ing 표현`) · 번호(`1. (자동사) …`) · 보조 설명(`→ …`) 이
 *  섞이고, 어휘 절이 아예 없는 것도 있다. 틀을 씌우면 그때마다 내용이 잘린다.
 *  화면이 줄머리를 보고 모양만 다르게 준다. */
export interface LessonTip {
  /** '토익 TIP' 아래부터 '어휘' 앞까지. 빈 줄은 뺀 뒤의 줄 목록 */
  body: string[]
  /** '어휘' 절만. 카드 아래쪽과 **핵심 어휘 모음**에 함께 쓰인다.
   *  본문 안에 섞여 있는 표현 짝(`be wearing ~을 착용하고 있다`)은 여기 넣지 않는다 —
   *  그건 팁의 설명이고, 시트가 '어휘' 라고 이름 붙인 것만 어휘로 센다. */
...
export interface RecapSentence {
  ...
  /** 한 문장에 빈칸이 **둘 이상**일 때, 칸 순서대로의 정답.
   *  ("주어가 하는 주체이면 ( ), 받는 대상이면 ( )를 쓴다" → ['능동태', '수동태'])
   *  빈칸이 하나면 비워 둔다 — 그때는 위 `answer`·`keywords` 가 그 칸의 것이다.
   *  ⚠️ 있을 때도 `answer`·`keywords` 는 **첫 칸** 값으로 채워 둔다(마이크 언어 판별처럼
   *     칸을 따지지 않는 자리가 그대로 돌게). */
  blanks?: { answer: string; keywords: string[] }[]
}
