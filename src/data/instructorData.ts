...
/* 🔬 지금은 **비워 둔 상태다** — 소리 앞뒤가 깎여 들리는 원인을 가리는 중이다.
   되돌리려면 아래 두 줄을 살리면 된다:
       lee_doyun: 0.93,
       yun_daeun: 1.05,   // v3 로 오면서 잃은 speed 1.1 을 조금 되돌린 값
   ⚠️ 속도를 1이 아닌 값으로 두면 브라우저가 **음정을 지키려고 시간을 늘였다 줄이는 처리**를
      건다(preservesPitch). 그 처리는 앞뒤에 버퍼가 필요해서, 채워지기 전(머리)과 비우지
      못한 채 끝난 자리(꼬리)가 깎인다 — 발화마다 앞뒤가 살짝 잘려 들리는 증상의 정체다.
      재생 쪽 여유(HEAD_WAIT_MS·TAIL_HOLD_MS)로는 못 막는다. 그건 타이밍이지 소리가 아니다. */
