...
/* ── 답을 고른 뒤 보기 **네 개 글자를 한꺼번에** 열 것인가 ──
   ...
   이도윤도 **간결본부터는 같다**(구현 중 메모 90행). 원래는 켜지 않았다 — "이번에는 A를
   볼게요" 하며 보기를 하나씩 들려주는 구성이라, 네 개가 미리 열려 있으면 지금 어느 보기
   이야기인지 화면에서 읽히지 않았다. 그런데 간결본이 **보기별로 들려주는 구간을 없애면서**
   하나씩 여는 근거가 같이 사라졌다. 지금은 글자만 순서대로 뜨는 셈이라 다 여는 편이 낫다.
