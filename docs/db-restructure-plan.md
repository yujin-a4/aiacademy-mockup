...
### 🟢 지문 세트 (이중·삼중 지문) — 0027 · 완료 (2026-08-07)

> **왜 뒤늦게 붙었나:** STEP 3(0014)은 `questions.passage_id` **하나**만 뒀다. Part 7 여덟 강 중
> 두 강(`RC-P7-07` 이중 · `RC-P7-08` 삼중)은 지문이 2~3개인데 2·3번을 붙일 자리가 없었다.
> 교재 적재기(`scripts/load-rc-questions.js`)가 그 두 강의만 못 넣고 있었다.

```sql
alter table passages add column set_code text, add column set_seq smallint default 1;
```

- 지문 행은 **지문 하나당 한 행** 그대로다(문장·메타가 각자 있어야 하므로). 같은 세트라는 사실만
  `set_code` 로 묶고, **문항은 세트의 첫 지문**(`set_seq=1`)을 가리킨다
- 조인표(`question_passages`)를 두지 않은 이유: 문항은 세트 전체를 보고 푼다. 연계 문항은 애초에
  두 지문을 다 본다. 조인표는 매번 손으로 채워야 하는데 화면은 결국 세트 전체를 그린다
- 읽는 쪽 `questionStore.ts` 가 `set_code` 로 형제 지문을 **한 번 더** 읽어 `question.passages` 에 붙인다.
  세트가 없는 강의는 그 쿼리를 아예 안 한다
- 화면은 이미 준비돼 있었다 — `ContentView` 의 `PassageTabs`. 막힌 건 DB 와 어댑터뿐이었다
- 강사가 지문 탭을 옮기는 근거: 적재기가 넣는 `content.evidence_passage`(근거가 몇 번 지문인지) ·
  `content.linked`(교재 해설이 '연계'라고 표시한 문항). 레일(시트)에는 지문 번호가 없다

**남은 것**

- [ ] 연계 문항의 `match`(근거 연결) 상호작용은 DB 레일 경로에서 아직 `mark` 로 낮춰진다 —
      "어느 지문의 어느 문장을 잇는가"가 레일에 없다(`fromSteps.ts` 의 기존 한계와 같은 자리)
