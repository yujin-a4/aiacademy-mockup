...
- ✅ **Phase 3-3 콘텐츠 이식 — D1 4개 ui_type 전부 DB 구동**(`src/data/typeLearning/fromDb.ts`).
  `/type-lesson/{t01,t07,t08,t09}`가 앵커 문항(`LC-P1-01-Q001`·`RC-P5-08-Q001`·`RC-P6-01-Q001`·
  `RC-P7-03-Q001`) 기준으로 지문·보기·근거·레일·세션정리까지 DB에서 생성된다. 로컬 TS는 폴백으로만 남음.
  ⚠️ 아직 로컬 `directiveOf()` 진행 — `/api/tutor` sheetRail 교체(3-3 본편)는 미착수.
  ⚠️ DB에 문장 해석(ko)이 없어 직독직해 탭이 비어 있다 → P7 레일은 '문장 탭' 대신 근거 표시(mark)로 우회.
