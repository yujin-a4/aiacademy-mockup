...
# 2026-09-03 — 이도윤 LC 만 '개념학습 추가 버전' 으로 옮겼다(RC 는 아직 옛 탭).
# ⚠️ 이 표는 강사 하나에 탭 하나라 **강의별로 탭이 갈리면 담지 못한다** — LC 기준으로 둔다.
# ⚠️ 생성기(build-fgi-scenario.js)와 **같은 탭을 봐야 한다.** 윤다은 정본이 '정오답분기'로
#    바뀐 뒤에도 여기가 '간결'을 보고 있어서, 멀쩡한 대본에 '발화가 시트에 없다'가 14건 떴다(09-08).
TABS = {'yun_daeun': 'FGI_윤다은_정오답분기', 'lee_doyun': 'FGI_이도윤 (개념학습 추가 버전)_1차 수정완료'}
...
for inst, byCode in sc.items():
    ...
    for code, les in byCode.items():
        ...
        # 예외 없이 글자 그대로 대조한다. ('실전 안내' 의 "총 N 문제" 는 시트가 LC 를 5로 적어 둬서
        #  한동안 여기만 눈감아 줬는데, 08-20 에 시트가 4로 고쳐져 예외를 걷어냈다 —
        #  생성기의 숫자 맞춤은 그대로 두되 이제 아무 일도 하지 않는다. 또 어긋나면 여기서 걸린다.)
